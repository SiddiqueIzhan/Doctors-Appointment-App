...
const PendingPayouts = ({ payouts }: PendingPayouts) => {
  ...
  const [approvePayoutDialog, setApprovePayoutDialog] =
  ...
  const handleApprovePayout = (payout: PayoutWithUser) => {
    setSelectedPayout(payout);
    setApprovePayoutDialog(true);
  };
  ...
  useEffect(() => {
    if (data && data.success) {
      setApprovePayoutDialog(false);
      setSelectedPayout(null);
      toast.success("Payout Successfully Approved");
    }
  }, [data]);
  ...
  return (
    <div>
          ...
          {payouts.length === 0 ? (
            ...
            <div className="space-y-4">
              {payouts.map((payout) => (
                <Card
                  key={payout.id}
                  className="bg-black border-emerald-900/20 hover:border-emerald-700/30 transition-all"
                >
                  <CardContent className="p-4">
                    <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 items-center">
                        <Badge
                          variant="outline"
                          className="bg-amber-900/20 border-amber-900/30 text-amber-400 py-2 px-4 font-bold"
                        >
                          PENDING
                        </Badge>
      ...
      {selectedPayout && !approvePayoutDialog && (
      ...
      {approvePayoutDialog && selectedPayout && (
        <Dialog
          open={approvePayoutDialog}
          onOpenChange={() => setApprovePayoutDialog(false)}
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-white">
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setApprovePayoutDialog(false)}
                disabled={loading}
                className="border-emerald-900/30"
              >
                Cancel
