...
const VideoCall = ({ sessionId, token }: VideoCallProps) => {
      ...
      sessionRef.current.connect(token, (error: Error) => {
        if (error) {
          ...
          if (publisherRef.current) {
            sessionRef.current.publish(publisherRef.current, (error: Error) => {
              if (error) {
  ...
  const toggleVideo = () => {
    if (publisherRef.current) {
      ...
      setVideoActive((val) => !val);
    }
  };

  const toggleAudio = () => {
    if (publisherRef.current) {
      publisherRef.current.publishAudio(!isAudioActive);
      setAudioActive((val) => !val);
    }
  };
      ...
      <div className="container mx-auto px-4">
        ...
        {isloading && !scriptLoaded ? (
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 h-100 gap-6">
              ...
              <div className="border border-emerald-900/20 rounded-lg">
                <div className="bg-emerald-900/10 px-3 py-2 text-emerald-400 text-sm font-medium">
                  Other Participant
                </div>
                <div
                ...
                >
                  {(!isConnected || !scriptLoaded) && (
                    <div className="flex items-center justify-center">
                      <div className="bg-muted/20 rounded-full p-8 my-20">
                        <User className="h-12 w-12 text-emerald-400" />
                      </div>
                    </div>
                  )}
            ...
            <div className="text-center">
              <p className="text-muted-foreground text-sm">
                ...
                {isAudioActive ? " Microphone on" : " Microphone off"}
              </p>
